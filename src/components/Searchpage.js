...
class Searchpage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      searchString: '', // at the beggining empty string
      searchedQuery: [], // data get from API search method
      foundBooks: []
    }
  }

  handleSearch = (query) => {
    BooksAPI.search(query).then(dataBooks => {
      if(dataBooks.error) {
        this.setState({
          searchedQuery: [],
          searchString: console.log(dataBooks.error)
        })
      }
      else {
        dataBooks.map(book => (this.state.foundBooks.filter((found) => found.id === book.id)
        ));
        this.setState({
          searchString: query,
          searchedQuery: dataBooks
        })
      }
    })
  }
...
// handleSearch => search(query) ... get this promise
// (20) [{…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}]
